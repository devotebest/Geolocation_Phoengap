...
MyApp.map_page_2 = function (params) {
 ...
 dismaland = [
  {
	...
	function onSuccess(position) {
		...
		if(first_time == 1) { 
			...
			map_2.setCenter(location);
			map_2.setZoom(16);
			first_time = 0;
		} else {
			locationMarker_2.setMap(null);
      		locationMarker_2 = null;
		}
		
		gpslocmarker = {
			url: 'images/map_icons/gpsloc.png'
		}
		locationMarker_2 = new google.maps.Marker({
			position:location,
			title: "You're here",
			icon: gpslocmarker,
			map: map_2
		});
        console.log(locationMarker_2);
	};
...
getCurrentLocation_2 = function()
{
    if(locationMarker_2 === null) {
          console.log('locationMarker_2 is null');  
        } else {
            console.log(locationMarker_2); 
        }
    ...
    if(locationMarker_2 === null) {  
        ...
        store_map1_watchID.load;
        console.log ('store_map1_watchID load');
        console.log(store_map1_watchID);
        
        navigator.geolocation.clearWatch(store_map1_watchID);
        watchID_2 = navigator.geolocation.watchPosition(onSuccess, onError);
        store_map2_watchID = watchID_2;
        
        console.log(watchID_2);
	...
	else
	{
		...
		navigator.geolocation.clearWatch(watchID_2);
		
		//map_2.setZoom(16);
		home_location = new google.maps.LatLng('51.477795,-2.599402');
		map_2.setCenter(home_location);
		first_time = 1;
	}
};	
...
{ /*
    ...
    */
	...
	var viewModel = {
		 ...
		 onReadyAction: function (e) {
			...
			*/
			...
			map_2.mapTypes.set('map_style', styledMap);
             ...
             viewModel.drawMarkersTheGoogleWay(map_2);
		...
		drawMarkersTheGoogleWay: function (map_local) {
   /* Moved to index.js
  ...
  */
			for (var key in mapMarkersTrail) {
				...
				if (mapMarkersTrail.hasOwnProperty(key)) {
                    //console.log([key]);
                    //console.log(mapMarkersTrail[key].id);
					//alert ('key = ' + key);
                    ...
                    infoWindowContent = '<div class="infoWindow infoWindowTrail">' + location_name + artists_locations[mapMarkersTrail[key].id]  + '</div>';
				...
				var image = {
                   ...
                   markersTrail_2.push(new google.maps.Marker({
                    ...
                    google.maps.event.addListener(markersTrail_2[key], 'click', function(innerKey) {
                        return function() {
                            viewModel.closeAllInfoWindows();
                            //console.log([innerKey]);
                            infowindowsTrail[innerKey].open(map_local, markersTrail_2[innerKey]);
                    ...
                    if (params.id == mapMarkersTrail[key].id) {
                        infowindowsTrail[key].open(map_local, markersTrail_2[key]);
                    }
                    
                    
                    /*
                    var infowindowsTrailKey = $.grep(mapMarkersTrail, function(e){ return e.id == mapMarkersTrail[key].id; });
                    //console.log(infowindowsTrailKey[0]);
                    */
		...
		closeAllInfoWindows: function () {
			for(var i=0; i<infowindowsTrail.length; i++){
				infowindowsTrail[i].close(map_2);
			}
            /*
